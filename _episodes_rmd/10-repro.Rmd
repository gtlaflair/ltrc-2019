---
title: "Part 5: Reproducible Reporting"
teaching: 40
exercises: 35
questions:
...
Now, we will work on creating a research report. There are a lot methods for doing this that range from "quick and easy" to more complicated with more formatting options (e.g. the `papaja` package for creating APA manuscripts). We are going to work with the quick and easy method :).
...
Let's save this file as `report.Rmd` in the documents folder. First we will compile it as is so that we can see how the knit button works. Some example minimal, inline formating that can be done is below. More can be found on [RStudio's Rmarkdown cheatsheet](https://resources.rstudio.com/rstudio-cheatsheets/rmarkdown-2-0-cheat-sheet).
...
```{r, purl=FALSE, message=FALSE, error=FALSE, comment=FALSE, warning=FALSE}
# read in data
library(tidyverse)
